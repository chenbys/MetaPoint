...
from ckit.hook_mseval import MSEvalHook, MSDistEvalHook
...
def train_model(model,
    ...
    if validate:
        ...
        dataloader_setting = dict(samples_per_gpu=1, workers_per_gpu=2, num_gpus=len(cfg.gpu_ids),
                                  dist=distributed, shuffle=False, pin_memory=False)
        dataloader_setting = dict(dataloader_setting, **cfg.data.get('val_dataloader', {}))
        val_dataloader = build_dataloader(val_dataset, **dataloader_setting)
        eval_hook = MSDistEvalHook if distributed else MSEvalHook
    ...
    if 'val_only' in cfg and cfg.val_only:
        runner.hooks[4].before_run(runner)
        runner.hooks[4]._do_evaluate(runner)
        return

...
class TransformerPoseTwoStage(BasePose):
    ...
    def forward_train(self, img_s, target_s, target_weight_s, img_q, target_q,
        ...
        target_keypoints = self.parse_keypoints_from_img_meta(img_metas, feature_q.device)  # [bs, num_query, 2]
    ...
    def forward_test(self,
        ...
        if self.with_keypoint:
            pred = output[-1].detach().cpu().numpy()  # [bs, num_query, 2]
            pred[~mask_s.cpu().bool()[:, :, 0]] = 0.5
            processed = self.keypoint_head.decode(img_metas, pred, img_size=[img_width, img_height])
            result['major'] = processed

            pred = output[0].detach().cpu().numpy()  # [bs, num_query, 2]
            pred[~mask_s.cpu().bool()[:, :, 0]] = 0.5
            processed = self.keypoint_head.decode(img_metas, pred, img_size=[img_width, img_height])
            result['ref0'] = processed

            pred = output[1].detach().cpu().numpy()  # [bs, num_query, 2]
            pred[~mask_s.cpu().bool()[:, :, 0]] = 0.5
            processed = self.keypoint_head.decode(img_metas, pred, img_size=[img_width, img_height])
            result['ref1'] = processed

            pred = initial_proposals.detach().cpu().numpy()  # [bs, num_query, 2]
            pred[~mask_s.cpu().bool()[:, :, 0]] = 0.5
            processed = self.keypoint_head.decode(img_metas, pred, img_size=[img_width, img_height])
            result['prop'] = processed

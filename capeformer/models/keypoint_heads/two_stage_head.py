...
class TwoStageHead(nn.Module):
    ...
    def forward(self, x, feature_s, target_s, mask_s):
        ...
        if self.support_embedding:
            ...
            support_order_embedding = support_order_embedding.transpose(
                0, 1)[None, :, :].repeat(bs, 1, 1).unsqueeze(-2)  # [bs, embed_dim, 1, num_query]
        else:
            masks = x.new_zeros((bs, 1, target_s[0].shape[1])).to(torch.bool)
            support_order_embedding = self.positional_encoding(masks)  # [bs, embed_dim, 1, num_query]
        ...
        if self.support_order_dropout != -1:
            ...
            support_order_embedding = torch.nn.functional.dropout2d(
                support_order_embedding, self.support_order_dropout, self.training)
            support_order_embedding = support_order_embedding.transpose(1, 3)
        ...
        query_embed_list = []
        for feature, target in zip(feature_s, target_s):  # resize the support feature back to the heatmap sizes.
            resized_feature = resize(input=feature, size=target.shape[-2:],
        ...
        return torch.stack(output_kpts, dim=0), initial_proposals, similarity_map  # [num_dec, bs, num_query, 2]
    ...
    def get_loss(self, output, initial_proposals, similarity_map, target,
        ...
        if target_weight.sum([1, 2]).min() == 0:
            d = 1
    ...
    def get_accuracy(self, output, target, target_weight, target_sizes):
        ...
        _, avg_acc, _ = keypoint_pck_accuracy(output, target,
                                              target_weight.astype(np.bool8),
                                              thr=0.2, normalize=target_sizes)
